...
class ReportGenerator {
  ...
  static ReportData buildReport({
    ...
    // Seat analytics
    final totalSeats = seats.isNotEmpty
        ? seats.length
        : ownerProfile.totalSeats;
    final occupiedSeats = seats
        .where((Seat s) => s.status == SeatStatus.occupied)
        .length;
    final maintenanceSeats = seats
        .where((Seat s) => s.status == SeatStatus.maintenance)
        .length;
    final reservedSeats = seats
        .where((Seat s) => s.status == SeatStatus.reserved)
        .length;
    final availableSeats =
        totalSeats - occupiedSeats - maintenanceSeats - reservedSeats;
    final occupancyPct = totalSeats > 0
    ...
    final List<Student> filtered = students.where((Student s) {
      if (isMonthly) {
        ...
        try {
          ...
          if (joinedDate != null) {
            return joinedDate.month == selectedMonth &&
                joinedDate.year == selectedYear;
          }
        } catch (_) {}
        return true;
      } else {
    ...
    final activeCount = students
        .where((Student s) => s.payment == PaymentStatus.paid)
        .length;
    final expiredCount = students
        .where((Student s) => s.payment == PaymentStatus.expired)
        .length;
    final pendingCount = students
        .where((Student s) => s.payment == PaymentStatus.pending)
        .length;

    // Revenue metrics calculation
    final totalRev = students.fold<double>(
      0.0,
      (sum, Student s) => sum + s.fee,
    );
    final pendingRev = students
        .where(
          (Student s) =>
              s.payment == PaymentStatus.pending ||
              s.payment == PaymentStatus.expired,
        )
    ...
    final dailyAdmissions = [
      DailyAdmissionRecord(
        date: '01 ${_months[selectedMonth - 1]}',
        count: 4,
        revenue: 4800,
      ),
      DailyAdmissionRecord(
        date: '05 ${_months[selectedMonth - 1]}',
        count: 3,
        revenue: 3600,
      ),
      DailyAdmissionRecord(
        date: '10 ${_months[selectedMonth - 1]}',
        count: 6,
        revenue: 7200,
      ),
      DailyAdmissionRecord(
        date: '15 ${_months[selectedMonth - 1]}',
        count: 2,
        revenue: 2400,
      ),
      DailyAdmissionRecord(
        date: '20 ${_months[selectedMonth - 1]}',
        count: 5,
        revenue: 6000,
      ),
    ];
    ...
    return ReportData(
      reportType: isMonthly ? 'Monthly' : 'Yearly',
      selectedPeriod: isMonthly
      ...
      totalCollection: totalRev,
      pendingFees: pendingRev,
      cashPayment: cashCol,
      upiPayment: upiCol,
      bankPayment: 0,
      studentList: filtered.isNotEmpty ? filtered : students,
      ...
      monthlyPlanCount: students
          .where((Student s) => s.membership == MembershipType.fullTime)
          .length,
      quarterlyPlanCount: 4,
      halfYearlyPlanCount: 2,
      yearlyPlanCount: 3,
      customPlanCount: students
          .where((Student s) => s.membership == MembershipType.halfTime)
          .length,
